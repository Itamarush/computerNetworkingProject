...
# create and send discoverPacket in broadcast
def dhcp():
    print("trying to find a DHCP Server...")
    discoverPacket = Ether(dst="ff:ff:ff:ff:ff:ff") / \
                    IP(src='0.0.0.0', dst='255.255.255.255') / \
                    UDP(sport=68, dport=67) / \
                    BOOTP(chaddr="00:15:5D:E6:64:07", xid=1) / \
                    DHCP(options=[("message-type", "discover"), "end"])
    time.sleep(0.5)
    sendp(discoverPacket, iface="enp0s3", verbose=0)
    print("sent DISCOVER message in order to find a DHCP server")
...
def handle_dhcp(packet):
    ...
    if DHCP in packet and packet[DHCP].options[0][1] == 2:
        ...
        request_packet = Ether(dst="ff:ff:ff:ff:ff:ff", src="00:15:5D:E6:64:07") / IP(src=ip, dst="255.255.255.255") / UDP(
            sport=68,
            dport=67) / BOOTP(
            op=1, chaddr="00:15:5D:E6:64:07".replace(":", ""), xid=RandInt()) / DHCP(options=[("message-type", "request"),
                                                                                     ("requested_addr", ip),
                                                                                     ("server_id", "255.255.255.255"),
                                                                                     ("hostname", "client"), "end"])

        sendp(request_packet, iface="enp0s3")
